...
class DataService:
    ...
    def _get_photo_from_provider(self, page_number):
        photos = []
    ...
    def execute_scout_data_generation_cycle(self, page_number):
        while True:
            try:
                for photo in self._get_photo_from_provider(page_number):
    ...
    def download_docs(self, collection_id):
        data = {}
        for doc in self.firebase.get_documents(collection_id):
    ...
    def upload_docs(self, collection_id):
        ...
        for key, value in data.items():
            self.firebase.add_document(collection_id=collection_id,
                                       document_id=key,
                                       data=value)

    def copy_geo_hash(self):
        for doc in self.firebase.get_documents(collection_id=SPOTS_REF_NAME):
            doc_id = doc['id']
            logging.info(f'Copying geohash for {doc_id}...')
            geo_doc = self.firebase.get_document(
            ...
            self.firebase.add_field_to_document(
                collection_id=SPOTS_REF_NAME,
                document_id=doc_id,
                field={'geohash': geohash}
            )
            wait_for_random_seconds(max=5)
